/* Source: https://opensourceui.in/components/3d-button
...
export type ThreeDButtonProps = Readonly<{ children: ReactNode; variant?: ThreeDButtonVariant; size?: ThreeDButtonSize } & ComponentPropsWithoutRef<"button">>;

const SIZE: Record<ThreeDButtonSize, string> = {
  sm: "h-9 gap-1.5 rounded-lg px-3 text-xs", md: "h-10 gap-2 rounded-lg px-4 text-sm",
  lg: "h-12 gap-2 rounded-xl px-5 text-sm", icon: "size-10 rounded-lg p-0",
};
...
const VARIANT: Record<ThreeDButtonVariant, string> = {
  ...
  soft: cn("bg-neutral-50 text-neutral-800 hover:text-neutral-900 active:bg-neutral-100", BEVEL_LIGHT, PRESSED_LIGHT),
  muted: cn("bg-neutral-100 text-neutral-700 hover:bg-neutral-200/80 hover:text-neutral-900 active:bg-neutral-200", BEVEL_LIGHT, PRESSED_LIGHT),
};

export const ThreeDButton = forwardRef<HTMLButtonElement, ThreeDButtonProps>(
  ({ className, children, variant = "solid", size = "md", type = "button", disabled, ...props }, ref) => (
    <button ref={ref} type={type} disabled={disabled} data-slot="3d-button" data-variant={variant} data-size={size}
      className={cn("inline-flex cursor-pointer items-center justify-center font-sans font-semibold outline-none select-none",
        ...
        SIZE[size], VARIANT[variant], size === "icon" && "shrink-0", className)} {...props}>{children}</button>
  ),
);
